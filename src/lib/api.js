...
async function req(url, options = {}) {
  ...
  // normalisation URL (évite double // ou erreurs)
  ...
  // 🔥 FIX IMPORTANT : gestion token plus fiable
  const isPublic = publicRoutes.some(route => cleanUrl.startsWith(route));
  ...
  // 🔥 gestion erreurs API
  if (!res.ok) {
    let message = data.message || data.erreur || `Erreur ${res.status}`;

    if (res.status === 404) {
      message = "Route API introuvable (vérifie BASE_URL ou backend)";
    }
...
// 🔥 pour download fichiers
function reqBlob(url) {
  const token = localStorage.getItem('accessToken');
  const cleanUrl = url.startsWith('/') ? url : `/${url}`;

  return fetch(`${BASE}${cleanUrl}`, {
    headers: token
      ? { Authorization: `Bearer ${token}` }
      : {},
  });
...
export const api = {
  ...
  executerRappels: (date) =>
    req('/rappels/executer', {
      method: 'POST',
      body: JSON.stringify({ date }),
    }),

  transfererFiche: (id, payload) =>
    req(`/nc/${id}/transferer`, {
      method: 'POST',
  ...
  supprimerColonnePerso: (id) =>
    req(`/colonnes-perso/${id}`, { method: 'DELETE' }),

  majValeursPerso: (ncId, valeurs) =>
    req(`/nc/${ncId}/valeurs-perso`, {
      method: 'PUT',
      body: JSON.stringify({ valeurs }),
    }),
};
