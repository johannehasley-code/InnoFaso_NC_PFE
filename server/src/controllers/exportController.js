...
//     if (req.query.limit)  { q +=  ` LIMIT ${Number(req.query.limit || 100)}`;  p.push(parseInt(req.query.limit)); }
//     if (req.query.offset) { q += ` OFFSET ${Number(req.query.offset || 0)}`; p.push(parseInt(req.query.offset)); }

//     const [rows] = await pool.execute(q, p);

//     await log({ userId: req.user?.id || null, action: 'SEARCH_NC', ipAddress: req.ip,
...
// // ── Export PDF fiche NC — reproduit l'étape 1 "Identification" du formulaire ──
...
//     const refDoc = textSafe(nc.refDocument) !== '—' ? nc.refDocument : 'PM-SM-EN-FNC-E';
...
//     row('N° de fiche', nc.numero);
//     row('Date / heure', fmt(nc.cree_le));
//     row('Émetteur', nc.emetteur);
//     row('Service concerné', nc.service);
//     y += 4;
//     doc.rect(40, y, 160, 60).fill('#D6E4F0');
//     doc.rect(200, y, W - 160, 60).fill(GREY);
//     doc.fillColor(BLUE).fontSize(9).font('Helvetica-Bold').text('Intitulé de la NC', 44, y + 4);
//     doc.fillColor('#222').font('Helvetica').fontSize(9)
//        .text(textSafe(nc.intitule), 204, y + 4, { width: W - 168, height: 52 });
//     y += 70;

//     // ── Bloc "Non-conformité Produit / Service" (fidèle au formulaire) ──
...
//     doc.fillColor('#19241d').font('Helvetica-BoldOblique').fontSize(11)
//        .text('Automatique', 40, y + 4, { width: W, align: 'center' });
//     y += 30;
...
//     await log({ userId: req.user?.id || null, action: 'EXPORT_NC_EXCEL', ipAddress: req.ip,
...
//     await log({ userId: req.user?.id || null, action: 'EXPORT_RAPPORT_MENSUEL', ipAddress: req.ip, newValue: { mois, annee, total } });
...
export const searchNC = async (req, res) => {
  try {
    ...
    if (req.query.statut) {
      q += ' AND statut = ?';
      p.push(req.query.statut);
    }

    if (req.query.criticite) {
      q += ' AND criticite = ?';
      p.push(req.query.criticite);
    }

    if (req.query.service) {
      q += ' AND service = ?';
      p.push(req.query.service);
    }
    ...
    // ✅ FIX PRODUCTION SAFE LIMIT/OFFSET
...
export const getExportStats = async (req, res) => {
  try {
    const [total] = await pool.execute('SELECT COUNT(*) AS n FROM nc');
    const [ouvertes] = await pool.execute(
      "SELECT COUNT(*) AS n FROM nc WHERE statut IN ('ouverte','en_cours')"
    );
    const [critiques] = await pool.execute(
      "SELECT COUNT(*) AS n FROM nc WHERE criticite='critique' AND statut != 'cloturee'"
    );
    const [cloturees] = await pool.execute(
      "SELECT COUNT(*) AS n FROM nc WHERE statut='cloturee'"
    );
    const [byCrit] = await pool.execute(
      'SELECT criticite, COUNT(*) AS n FROM nc GROUP BY criticite'
    );
    const [byService] = await pool.execute(
      'SELECT service, COUNT(*) AS n FROM nc GROUP BY service ORDER BY n DESC LIMIT 5'
    );
...
export const exportNcPDF = async (req, res) => {
  try {
    ...
    const refDoc =
      textSafe(nc.refDocument) !== '—' ? nc.refDocument : 'PM-SM-EN-FNC-E';
    ...
    section('1. IDENTIFICATION');
    row('N° de fiche', nc.numero);
    row('Date', fmt(nc.cree_le));
    row('Émetteur', nc.emetteur);
    row('Service', nc.service);
...
export const exportRapportMensuel = async (req, res) => {
  try {
    ...
    const [rows] = await pool.execute(
      'SELECT * FROM nc WHERE cree_le BETWEEN ? AND ?',
      [debut, fin]
    );
