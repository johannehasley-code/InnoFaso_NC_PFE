...
for (const u of USERS) {
  ...
  if (!roles.length) { console.warn(`Role "${u.role}" introuvable, skip.`); continue; }
  await conn.execute(
    `INSERT INTO users (nom, prenom, email, password_hash, role_id, actif)
     VALUES (?,?,?,?,?,1) ON DUPLICATE KEY UPDATE password_hash=?, actif=1`,
